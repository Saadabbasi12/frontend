...
export default function AssistantPanel({ position }) {
  ...
  const handleCommand = (text) => {
    ...
    if (!job || !job.candidates) {
      reply(
        "No job or candidates found. Please create a job with candidates first.",
      );
      return;
    }
    ...
    if (text.includes("generate") && text.includes("rubric")) {
      ...
      reply(
        "✅ Evaluation rubric generated! I've auto-filled 5 criteria with weighted scoring to match this role's requirements.",
      );
      return;
    }
    ...
    if (text.includes("shortlist") && text.includes("candidate")) {
      ...
      reply(
        `✅ Shortlisted ${shortlistedCount} top candidates: ${candidateNames}. They've all been moved to the Shortlisted stage based on their scores.`,
      );
      return;
    }
    ...
    if (text.includes("schedule") && text.includes("interview")) {
      ...
      reply(
        "📅 Opening interview scheduler. Select a candidate and time slot to schedule their interview.",
      );
      return;
    }
    ...
    reply(
      "I didn't quite understand that. Try asking me to shortlist candidates, generate a rubric, or schedule an interview.",
    );
  };
